...
export function SectionHeader({
  icon, label, title, action, actionHref,
}: {
  icon: React.ReactNode; label: string; title: string; action?: string; actionHref?: string;
}) {
...
export function NovelGridCard({ novel, locale }: { novel: SiteNovel; locale: string }) {
  const { theme } = useTheme();
  const ts = THEME_STYLES[theme];
...
export function NovelListCard({ novel, locale }: { novel: SiteNovel; locale: string }) {
  const { theme } = useTheme();
  const ts = THEME_STYLES[theme];
  const [hovered, setHovered] = useState(false);
  const isDark = theme !== "light";

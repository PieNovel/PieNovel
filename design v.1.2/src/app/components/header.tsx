    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14 gap-4">
          ...
          {/* Logo */}
          <Link to="/" className="flex items-center gap-0 flex-shrink-0 select-none group">
            <span style={{
              ...
              {langDropdown.open && (
                ...
                >
                  ...
                  {LANGUAGES.map((l) => (
                    <button
                      key={l.code}
                      onClick={() => { onLanguageChange(l.code); langDropdown.setOpen(false); }}
                      className="w-full flex items-center justify-between px-3 py-2 transition-all"
                      style={{
                        fontFamily: "'Inter', sans-serif",
                        fontSize: "0.82rem",
                        color: language === l.code ? "#10b981" : itemText,
                        background: language === l.code ? "rgba(16,185,129,0.1)" : "transparent",
                        margin: "0 6px",
                        width: "calc(100% - 12px)",
                        borderRadius: "10px",
                      }}
                      onMouseEnter={(e) => { if (language !== l.code) e.currentTarget.style.background = itemHoverBg; }}
                    ...
                    >
                      ...
                      {language === l.code && <Check className="size-3 flex-shrink-0" style={{ color: "#10b981" }} />}
                    </button>
                  ))}
                </div>
              )}
              ...
              {themeDropdown.open && (
                ...
                >
                  ...
                  {THEMES.map((t) => (
                    <button
                      key={t.id}
                      onClick={() => { onThemeChange(t.id); themeDropdown.setOpen(false); }}
                ...
                {userDropdown.open && (
                  ...
                  >
                    ...
                    {[
                      ...
                      { label: "My Library", icon: Heart, to: "/library" },
                      { label: "History", icon: Clock, to: "/history" },
                      { label: "Settings", icon: Settings, to: "/settings" },
                      ...(user.role === "admin" ? [{ label: "Admin Panel", icon: ShieldCheck, to: "/admin" }] : []),
                    ...
                    <div className="border-t mt-1 pt-1" style={{ borderColor: popupDivider }}>
                      <button
                        onClick={() => { logout(); userDropdown.setOpen(false); navigate("/"); }}
